 ...
 *
 * DISCO
   - Sometimes the knight can go to a new spot but not delete its old position.
   - This in turn could lead to there not being the amount of moves as the dimension size.
   - We have to be careful about how we define out of bounds because if we say not equal to -1. then it can start putting things on 
     tiles which already have knights, which can lead to errors. 
 *
 * QCC
  - Is the time it takes to get a Knight Tour roughly the same amongst different groups?
  - Would it skew the data if not all possible knight paths were stated?
  - Why does n and _sidelength both have to be defined?
 *
